...
# Add the project root to path for imports (adjust as needed)
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.insert(0, project_root)

# Import configuration and agent setup
try:
    from time_series_auto_labeling_agent import agent_executor
except ImportError:
    print("Warning: Could not import agent_executor. Using mock implementation.")
    ...
    class MockAgent:
        def invoke(self, input_data):
            return {
                ...
                {
                    "identified_segments": [
                        {"start_row": 200, "end_row": 250},
                        {"start_row": 400, "end_row": 450},
                        {"start_row": 600, "end_row": 650}
                    ],
                    "assigned_label": "Pattern_A",
                    "explanation": "These segments show similar cyclical patterns with peak-to-trough transitions.",
                    "uncertainty_notes": "Medium uncertainty due to noise in the data."
                }
                ```"""
            }
    agent_executor = MockAgent()

# Optional S3 imports - only loaded if S3 is enabled
if os.environ.get('USE_S3_STORAGE', 'false').lower() == 'true':
    try:
        ...
        print("Warning: S3 storage enabled but boto3 not installed. Falling back to local storage.")
        s3_enabled = False
else:
    s3_enabled = False
...
ALLOWED_EXTENSIONS = {'csv', 'txt', 'xlsx', 'xls', 'json'}
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['MAX_CONTENT_LENGTH'] = 50 * 1024 * 1024  # 50MB max upload size

# Default dataset path (used if no custom dataset is uploaded)
DEFAULT_DATASET_PATH = os.environ.get('DATASET_PATH', 'data/sample_timeseries.csv')
...
def load_dataset(file_path):
    """Load a dataset from file path based on file extension"""
    try:
...
def download_from_s3(bucket, s3_file_name, local_path):
    """Download a file from an S3 bucket"""
    try:
        s3_client.download_file(bucket, s3_file_name, local_path)
        return local_path
    except NoCredentialsError:
        logger.error("AWS credentials not available")
        raise Exception("AWS credentials not available")
...
# Load the default dataset on startup
try:
    df = load_dataset(DEFAULT_DATASET_PATH)
    columns = df.columns.tolist()
    current_dataset_path = DEFAULT_DATASET_PATH
    logger.info(f"Default dataset loaded successfully with {len(df)} rows and columns: {columns}")
except Exception as e:
...
def dashboard():
    ...
    if df is not None:
        stats = {
            ...
            'dataset_name': os.path.basename(current_dataset_path) if current_dataset_path else 'No dataset loaded'
        }
...
@requires_auth
def upload_dataset():
    global df, columns, current_dataset_path
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            if s3_enabled:
                try:
                    s3_key = f"datasets/{unique_filename}"
                    s3_path = upload_to_s3(file_path, S3_BUCKET, s3_key)
                    logger.info(f"File also uploaded to S3: {s3_path}")
                except Exception as e:
                    logger.error(f"Error uploading to S3: {e}")
                    flash(f"File saved locally but S3 upload failed: {str(e)}", "warning")
            ...
            try:
                ...
                flash(f'Dataset {filename} uploaded and loaded successfully with {len(df)} rows and {len(columns)} columns.')
                return redirect(url_for('dataset_details', dataset_id=dataset_id))
            
            except Exception as e:
                logger.error(f"Error loading uploaded dataset: {e}")
    ...
    # Get list of already uploaded datasets
    uploaded_datasets = []
    try:
        ...
        logger.error(f"Error listing uploaded datasets: {e}")
...
def dataset_details(dataset_id):
    if dataset_id not in loaded_datasets:
        flash('Dataset not found')
        return redirect(url_for('upload_dataset'))
    
    dataset_info = loaded_datasets[dataset_id]
    
    # Get dataset statistics
    try:
        dataset_df = load_dataset(dataset_info['path'])
        
        # Calculate basic statistics
        stats = {
            'rows': len(dataset_df),
            'columns': len(dataset_df.columns),
            'numeric_columns': len(dataset_df.select_dtypes(include=[np.number]).columns),
            'null_percent': round(dataset_df.isnull().mean().mean() * 100, 2),
            'memory_usage': f"{dataset_df.memory_usage(deep=True).sum() / (1024 * 1024):.2f} MB"
        }
        
        # Create column preview with types and sample values
        columns_preview = []
        for col in dataset_df.columns:
            col_info = {
                'name': col,
                'type': str(dataset_df[col].dtype),
                'nulls': f"{dataset_df[col].isnull().sum()} ({round(dataset_df[col].isnull().mean() * 100, 2)}%)",
                'sample': str(dataset_df[col].iloc[0]) if len(dataset_df) > 0 else 'N/A',
                'is_numeric': pd.api.types.is_numeric_dtype(dataset_df[col])
            }
            columns_preview.append(col_info)
        
        # Generate data preview
        data_preview = dataset_df.head(10).to_html(classes='table table-striped table-sm', index=True)
        ...
        logger.error(f"Error generating dataset details: {e}")
        flash(f'Error analyzing dataset: {str(e)}', 'danger')
        return redirect(url_for('upload_dataset'))

@app.route('/dataset/use/<dataset_id>')
@requires_auth
def use_dataset(dataset_id):
    ...
    try:
        ...
        columns = df.columns.tolist()
        current_dataset_path = dataset_info['path']
        
        flash(f'Now using dataset: {dataset_info["name"]}')
        return redirect(url_for('dashboard'))
    
    except Exception as e:
        logger.error(f"Error switching to dataset: {e}")
        flash(f'Error loading dataset: {str(e)}', 'danger')
        return redirect(url_for('upload_dataset'))
